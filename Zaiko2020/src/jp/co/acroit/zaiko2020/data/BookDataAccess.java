...
public class BookDataAccess {
	...
	public List<Book> find(SearchCondition sc) throws SQLException {
		...
		try {
			...
			String page = sc.getPage();						//表ページ数の取得
			...
			// LIMIT句の生成・加筆
			query = query + " LIMIT " + String.valueOf((Integer.parseInt(page) - 1) * 200) + ", 200";
...
//				digest = rs.getString(passwordColumn);
				...
				Book aBook = new Book(dbId, dbBookName, dbPublisher, dbAuthor, dbIsbn, dbSalsDate, dbPrice, dbStock, dbDeleteflg);
				bookList.add(aBook);
			}
